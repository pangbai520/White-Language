...
func main() -> Int {
    ...
    let name_value -> json.Value = json.string("White Language")?;
        ...
        builtin.print("FAIL: JSON value construction, error " + Int(err));
        return 1;
    }
    root.set("name", name_value)?;
    catch(err) {
        builtin.print("FAIL: JSON object construction, kind " + Int(root.kind()) + ", error " + Int(err));
        return 1;
    }
    root.set("year", json.integer(2026L)?)?;
    catch(err) {
        builtin.print("FAIL: JSON number construction");
        return 1;
    }
    ...
    values.append(json.boolean(true))?;
        ...
        builtin.print("FAIL: JSON array construction");
        return 1;
    }
    root.set("values", values)?;
    catch(err) {
        builtin.print("FAIL: JSON object insertion");
        return 1;
    }
    ...
    let root_length -> Int = root.length()?;
        ...
        builtin.print("FAIL: JSON object length");
        return 1;
    }

    if (name != "White Language" || year != 2026L || !first || root_length != 3) {
        builtin.print("FAIL: JSON value model");
        return 1;
    }

    builtin.print("PASS: JSON value model");
    return 0;
}
