...
Built from the ground up and fully bootstrapped (the WhiteLang compiler `wlc` is written entirely in WhiteLang), it aims to hit the sweet spot between low-level memory control and high-level ergonomics.

## Key Features

* **Self-Hosted & Independent**: The compiler has successfully passed the bootstrapping milestone. WhiteLang compiles WhiteLang.
...
* **Versus C**: WhiteLang eliminates the need for header files and manual `malloc`/`free` for strings and vectors thanks to built-in ARC, making it vastly safer for everyday data manipulation while retaining the ability to drop down to raw pointers.
* **Versus Rust**: WhiteLang avoids the steep learning curve of the borrow checker. You get deterministic memory management via ARC without spending hours fighting the compiler over lifetimes, making prototyping much faster.
* **Versus Python/Scripting**: WhiteLang is strictly statically typed and compiled ahead-of-time (AOT) to native machine code. It catches errors at compile-time and executes orders of magnitude faster.

## Limitations & Known Issues

WhiteLang is a passion project and a work in progress. If you are considering using it for production, please be aware of the following architectural and developmental limitations:
...
5. **Basic Error Handling**: WhiteLang does not currently have a robust `try/catch` exception mechanism or Monadic error handling (like `Result<T, E>`). Error management heavily relies on manual checking and C-style return codes.
...
We believe a language is only as good as its tooling. WhiteLang comes with first-class developer experience tools out of the box:
...
Since WhiteLang is self-hosted, you need an existing WhiteLang compiler (Stage 1 or the pre-compiled binary installer) to build the compiler from source.

### 1. Set Environment Variable
Ensure your `WL_PATH` is set to the root directory of the WhiteLang standard library:
```bash
...
WhiteLanguage is licensed under the Apache License 2.0 - see the [LICENSE](LICENSE) file for details.